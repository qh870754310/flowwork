package com.qh.modules.activiti.controller;

import com.qh.modules.activiti.dto.ProcessNodeDto;
...
import com.qh.modules.activiti.service.*;
...
public class ExtendActDealController {
    ...
    public String queryFlowsByActKey(Model model, String actKey, String busId) {
        List<Map<String, Object>> defs = actModelService.queryFlowsByActKey(actKey);
    ...
    public Result getStartFlowInfo(String deployId) {
        ...
        try {
            result = actModelService.getStartFlowInfo(deployId);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public String userWindow(String nodeId, String nodeAction, HttpServletRequest request, UserEntity user, Model model){
        int pageNum = Utils.parseInt(request.getParameter("pageNum"), 1);
        Page<UserEntity> mapPage = actModelService.userWindowPage(nodeId, pageNum, user.getUserName());
    ...
    public String turnUserWindow(UserEntity user, HttpServletRequest request, Model model){
        int pageNum = Utils.parseInt(request.getParameter("pageNum"), 1);
        Page<UserEntity> mapPage = actModelService.turnWindowPage(pageNum,user);
        model.addAttribute("page",mapPage);
        //1 为单选 2为复选
    ...
    public Result startFlow(ProcessTaskDto processTaskDto){
        Result result = null;
        try {
            actModelService.startFlow(processTaskDto);
    ...
    public Result showFlowImg(String processInstanceId, HttpServletResponse response){
        try {
            InputStream inputStream = actModelService.getFlowImgByInstantId(processInstanceId);
            //输出到页面
    ...
    public String myUpcoming(Model model, String code, String busId, HttpServletRequest request){
        ...
        Page<ExtendActModelEntity> page = actModelService.findMyUpcomingPage(params, pageNum);
        model.addAttribute("page",page);
    ...
    public String myDoneList(Model model, String code, String busId, HttpServletRequest request){
        ...
        params.put("code",code);
        params.put("busId",busId);
        Page<ExtendActModelEntity> page = actModelService.myDonePage(params, pageNum);
        model.addAttribute("page",page);
        model.addAttribute("code",code);
        model.addAttribute("busId",busId);
    ...
    public Result getChangeFileds(ProcessTaskDto processTaskDto){
        ...
        //查询需要作为流程条件判断的字段
        Set<String> nextVarNams = actModelService.getNextVarNams(task.getTaskDefinitionKey(),processTaskDto.getDefId());
        String[] changFile={};
    ...
    public Result getNextActNodes(ProcessTaskDto processTaskDto){
        List<ProcessNodeDto> nextActNodes = actModelService.getNextActNodes(processTaskDto);
    ...
    public Result doActTask(ProcessTaskDto processTaskDto,HttpServletRequest request){
        ...
        try {
            ...
            actModelService.doActTask(processTaskDto,params);
    ...
    public Result backStartUser(ProcessTaskDto processTaskDto,HttpServletRequest request){
        ...
        try {
            ...
            actModelService.endFailFolw(processTaskDto,params);
    ...
    public Result turnToDo(ProcessTaskDto processTaskDto,String toUserId,HttpServletRequest request){
        Result result;
        try {
            actModelService.turnToDo(processTaskDto,toUserId);
