...
import com.qh.modules.activiti.dao.ExtendActModelDao;
import com.qh.modules.activiti.entity.ExtendActModelEntity;
import com.qh.modules.activiti.service.ActModelService;
import com.qh.modules.activiti.service.ExtendActModelService;
...
//		extendActModelEntity.setBapidList(UserUtils.getDateAuth(Constant.DataAuth.BAP_DATA.getValue()));
    ...
    public String save(ExtendActModelEntity extendActModel) throws Exception {
        ...
        return actModelService.CreateModel(extendActModel);
    }
    ...
    public int delete(String id) {
        ...
        Model model = repositoryService.getModel(actModelEntity.getModelId());
    ...
    public void deploy(String modelId) throws IOException {
        ...
        //设置流程属性
        ExtendActModelEntity extendActModel = extendActModelDao.getModelAndBusInfo(modelId);
        //        String key=StringUtils.toStringByObject(extModel.getActKey())+"_"+modelId;
        String key = StringUtils.toStringByObject(extendActModel.getActKey());
        process.setId(key);
        process.setName(StringUtils.toStringByObject(extendActModel.getName()));
        process.setDocumentation(StringUtils.toStringByObject(extendActModel.getRemark()));
        ObjectNode objectNode = new BpmnJsonConverter().convertToJson(bpmnModel);
        //更新视图模型
        model.setName(StringUtils.toStringByObject(extendActModel.getName()));
        model.setKey(key);

        //metaInfo
        ObjectNode modelObjectNode = objectMapper.createObjectNode();
        modelObjectNode.put(ModelDataJsonConstants.MODEL_NAME, StringUtils.toStringByObject(extendActModel.getName()));
        modelObjectNode.put(ModelDataJsonConstants.MODEL_REVISION, model.getVersion());
        modelObjectNode.put(ModelDataJsonConstants.MODEL_DESCRIPTION, StringUtils.toStringByObject(extendActModel.getRemark()));
        ...
        actModelEntity.setId(extendActModel.getId());
