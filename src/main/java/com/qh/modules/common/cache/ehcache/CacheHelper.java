...
public class CacheHelper extends UCacheManager {
    ...
    public static CacheHelper getCache(String cacheName) {
        CacheHelper cacheHelper = (CacheHelper) helpers.get(cacheName);
        if (cacheHelper == null) {
            cacheHelper = new CacheHelper(cacheName);
            helpers.put(cacheName, cacheHelper);
        }
        return cacheHelper;
    }
